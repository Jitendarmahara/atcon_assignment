...
// A single retry-after-refresh: if the access token is expired, silently
// exchange the refresh token once and replay the original request, so a
// short-lived 15-minute access token doesn't interrupt a recruiter mid-task.
async function request<T>(path: string, options: RequestInit = {}, allowRefresh = true): Promise<T> {
  ...
  if (res.status === 401 && tokens?.refreshToken && allowRefresh) {
    ...
    setTokens(null);
    if (!path.startsWith("/public")) window.location.assign("/login");
